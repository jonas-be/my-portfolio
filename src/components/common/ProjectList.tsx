...
const ProjectList = ({baseUrl, list}: Props) => {
    ...
    return (
        <>
            ...
            <ul>
                {list.map(i =>
                    <li key={i.primaryLink} className="flex flex-col odd:md:flex-row even:md:flex-row-reverse items-center md:justify-between m-4 p-4 pb-16 rounded-xl">
                        <Link href={baseUrl + "/" + i.primaryLink} className="px-10">
                            <img src={i.imgUrl} alt="symbolic project icon" width="400" height="0"/>
                        </Link>
                        <div className="w-full md:p-2 md:w-[80vw] lg:w-[50vw] xl:w-[30vw]">
                            <h3 className="py-2">{i.title}</h3>
                            <ReactMarkdown className="text" children={i.contentSEO}/>
                            <Link href={baseUrl + "/" + i.primaryLink}
                                  className="btn btn-xs gap-2 btn-bordered border-accent-200 dark:border-accent-200-dark text-accent my-4 group/edit">
                                Mehr lesen
