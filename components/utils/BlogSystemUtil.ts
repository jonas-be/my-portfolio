...
export type ProjectLinkMapping = { id: any, title: string, primaryLink: string, links: string[], imgUrl: string, contentSEO: string };
...
export async function getBlogId(context: GetServerSidePropsContext, path: string): Promise<ProjectLinkMapping> {
    ...
    if (projects.list === undefined)
        return {id: -1, title: "Not Found", primaryLink: "not-found", links: ["not-found"], imgUrl: "", contentSEO: ""}
